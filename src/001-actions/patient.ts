...
export async function createPatient(
  ...
  const response = await axios.post<Patient>(`${API_URL}/api/patients`, data)
  return response.data
}

// Fetch patients by doctor
export async function fetchPatientsByDoctor(doctorId: string): Promise<Patient[]> {
  const response = await axios.get<Patient[]>(`${API_URL}/api/patients/doctors/${doctorId}/patients`)
  return response.data
}

// actions/patient.ts
export async function fetchPatientById(patientId: string): Promise<Patient> {
  const response = await axios.get<Patient>(`${API_URL}/api/patients/${patientId}`)
  return response.data
}
