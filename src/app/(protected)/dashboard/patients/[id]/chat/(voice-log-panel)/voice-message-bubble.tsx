...
interface VoiceMessageBubbleProps {
  ...
  onRetry: (rec: DisplayRecording) => void
}
  ...
  const renderStatus = (): React.ReactNode => {
    ...
    return (
      <div className='flex items-center space-x-2'>
        <div className='w-4 h-4 flex items-center justify-center rounded-full bg-red-600 text-muted text-[10px] font-bold'>
          !
        </div>

        {rec.status === 'failed' && rec.errorCode !== 'no_speech_detected' && (
          <button
            onClick={() => onRetry(rec)}
